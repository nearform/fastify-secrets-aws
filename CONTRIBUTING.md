# Welcome to Fastify Secrets AWS!
...
In order to run the tests you will need access to a AWS Secret Manager instance. Refer to the `Local dev environment` section of the readme on how to set the environment up.

Then run the tests with something like:

```
AWS_SDK_LOAD_CONFIG=1 npm test
```

After you have stored the credentials of the user in your local AWS configuration file.
